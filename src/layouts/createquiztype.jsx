...
// function Createquiztype({ quizName, quizType, setQuizType }) {
//   const [timer, setTimer] = useState("off");
//   const [questions, setQuestions] = useState([
//     { question_name: "", options: [{ text: "", imgUrl: "" }], correct_option: -1, type: "text" },
//   ]);
...
//     const newQuestions = questions.map((q, i) =>
//       i === index ? { ...q, options: [...q.options, { text: "", imgUrl: "" }] } : q
//     );
//     setQuestions(newQuestions);
//   };
...
//       title: quizName,
//       duration: durationMapping[timer],
...
//             <div
//               key={i}
//               className={`question-number-container ${i === activeQuestion ? "active" : ""}`}
...
//                       <div
//                         key={j}
//                         className={`option ${q.correct_option === j ? "correct" : ""}`}
//                         style={{
//                           backgroundColor: q.correct_option === j ? "green" : "transparent",
...
//                           style={{
//                             backgroundColor: q.correct_option === j ? "green" : "white",
//                           }}
//                         />
//                         <input
//                           type="text"
...
//                             onChange={(e) => handleOptionChange(i, j, e.target.value, "imgUrl")}
//                           />
//                         )}
//                         {j >= 2 && (
//                           <div
...
function Createquiztype({ quizName, quizType, setQuizType }) {
  ...
  const [questions, setQuestions] = useState([
    { question_name: "", options: [{ text: "", imgUrl: "" }], correct_option: -1 }
  ]);
  const [activeQuestion, setActiveQuestion] = useState(0);
    ...
    for (const question of questions) {
      ...
      for (const option of question.options) {
        if (option.text && !option.imgUrl) {
          setError("Each option must have valid Text");
          return;
        }
        if (option.imgUrl && !option.text) {
          setError("Each option must have valid Image URL.");
          return;
        }
        if (optionType === "text-image") {
          if (!option.text && !option.imgUrl) {
            setError("Each option must have valid Text and Image URL.");
            return;
          }
        }
      }
                      ...
                      >
                        ...
                        {optionType === "text" && (
                          <input
                            ...
                            value={opt.text}
                            placeholder="Text"
                            onChange={(e) => handleOptionChange(i, j, e.target.value, "text")}
                          />
                        )}
                        {optionType === "image" && (
                          <input
